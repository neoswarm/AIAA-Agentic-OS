...
@_login_required_api
def rotate_token():
    """Rotate the default setup-token profile token value."""
    if not _should_use_gateway_profile_api():
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "Gateway profile API is unavailable for rotate.",
                }
            ),
            400,
        )
    ...
    current_token = str(payload.get("current_token") or "").strip()

    if not new_token:
        return jsonify({"status": "error", "message": "new_token is required"}), 400

    existing_token = get_claude_token()
    if current_token and current_token != existing_token:
        return jsonify({"status": "error", "message": "Current token does not match"}), 409

    validation = validate_claude_token(new_token)
    if validation["status"] in ("invalid", "expired", "unsupported"):
        return (
            jsonify(
                {
    ...
    try:
        gateway_profile = _upsert_gateway_profile(CLAUDE_TOKEN_PROFILE_ID, new_token)
    except GatewayHTTPError as exc:
        return (
            jsonify(
                {
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"Gateway profile rotate failed: {exc}",
                }
            ),
            502,
        )
    ...
    if gateway_validation:
        validation = {
            ...
            "message": str(
                gateway_profile.get("message")
                or validation.get("message")
                or "Gateway profile rotate completed"
            ),
        }

    os.environ["CLAUDE_SETUP_TOKEN"] = new_token
    models.set_setting(CLAUDE_TOKEN_SETTING_KEY, new_token)
    persisted = _persist_to_railway_async({"CLAUDE_SETUP_TOKEN": new_token})
    init_chat_runner()
    ...
    _log_token_lifecycle(
        ...
        profile_id=CLAUDE_TOKEN_PROFILE_ID,
        persisted_to_railway=persisted,
        redacted=_redact_token(new_token),
    )
